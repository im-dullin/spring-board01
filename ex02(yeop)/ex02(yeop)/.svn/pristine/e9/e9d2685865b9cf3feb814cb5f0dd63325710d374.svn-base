...
<body>
	...
	<form method="post">
		...
		<input name="title" id="title" value="${boards.title}">

		<h4><label for="content">content</label></h4>
		<textarea rows="20" cols="40" name="content" id="content">${boards.content}</textarea>
		
		<h4><label for="writer">writer</label></h4>
		<input name="writer" id="writer" value="${boards.writer}">
		<input type="hidden" name="pageNum" value="${cri.pageNum}">
		<input type="hidden" name="amount" value="${cri.amount}">
		<input type="hidden" name="tyoe" value="${cri.type}">
